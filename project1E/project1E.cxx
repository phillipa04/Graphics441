...
double calculatePhongShading(LightingParameters lp, double *viewDirection, double *normal){
  double diffuseShade  = fabs(dotProd(lp.lightDir, normal));
  double coef = 2.0*dotProd(lp.lightDir, normal);
  double newVect[3];
  for(int i = 0; i<3; i++){
    newVect[i] = coef*normal[i];
  }
  double R[3];
  R[0] = newVect[0] - lp.lightDir[0];
  R[1] = newVect[1] - lp.lightDir[1];
  R[2] = newVect[2] - lp.lightDir[2];
  double specularShade = fmax(0, pow(dotProd(R, viewDirection), lp.alpha));
  return (lp.Ka + lp.Kd*diffuseShade + lp.Ks*specularShade);
}
...
void writeFlatBottomTriangle(Triangle triangle, unsigned char *buffer, int width, int height, double* zVals){
  ...
  double currentRed, currentGreen, currentBlue, currentNormX, currentNormY, currentNormZ;
  double currentLeftNormX, currentRightNormX, currentLeftNormY, currentRightNormY, currentLeftNormZ, currentRightNormZ;
  double baseLeftNormX, baseLeftNormY, baseLeftNormZ, baseRightNormX, baseRightNormY, baseRightNormZ, peakNormX, peakNormY, peakNormZ;
  ...
  if(triangle.Y[0] == triangle.Y[1]){
    ...
    if(triangle.X[0] < triangle.X[1]){
      ...
      baseLeftNormX = triangle.normals[0][0];
      baseLeftNormY = triangle.normals[0][1];
      baseLeftNormZ = triangle.normals[0][2];
      ...
      baseRightNormX = triangle.normals[1][0];
      baseRightNormY = triangle.normals[1][1];
      baseRightNormZ = triangle.normals[1][2];
    }
    else{
      ...
      baseLeftNormX = triangle.normals[1][0];
      baseLeftNormY = triangle.normals[1][1];
      baseLeftNormZ = triangle.normals[1][2];
      ...
      baseRightNormX = triangle.normals[0][0];
      baseRightNormY = triangle.normals[0][1];
      baseRightNormZ = triangle.normals[0][2];
    }
    peakX = triangle.X[2];
    peakY = triangle.Y[2];
    peakZ = triangle.Z[2];
    ...
    peakNormX = triangle.normals[2][0];
    peakNormY = triangle.normals[2][1];
    peakNormZ = triangle.normals[2][2];
  }
    ...
    if(triangle.X[0] < triangle.X[2]){
      ...
      baseRightNormX = triangle.normals[2][0];
      baseRightNormY = triangle.normals[2][1];
      baseRightNormZ = triangle.normals[2][2];
    }
    else{
      ...
      baseLeftNormX = triangle.normals[2][0];
      baseLeftNormY = triangle.normals[2][1];
      baseLeftNormZ = triangle.normals[2][2];
    ...
    peakNormX = triangle.normals[1][0];
    peakNormY = triangle.normals[1][1];
    peakNormZ = triangle.normals[1][2];
  }
  else{
    ...
    peakNormX = triangle.normals[0][0];
    peakNormY = triangle.normals[0][1];
    peakNormZ = triangle.normals[0][2];
  }
  ...
  for(int i = ymin; i <= ymax; i++){
    ...
    if(peakNormX == baseLeftNormX){
      currentLeftNormX = baseLeftNormX;
    }
    else{
      currentLeftNormX = baseLeftNormX + ((peakNormX - baseLeftNormX)/(peakY - baseLeftY))*((double) i - baseRightY);
    }
    if(peakNormY == baseLeftNormY){
      currentLeftNormY = baseLeftNormY;
    }
    else{
      currentLeftNormY = baseLeftNormY + ((peakNormY - baseLeftNormY)/(peakY - baseLeftY))*((double) i - baseRightY);
    }
    if(peakNormZ == baseLeftNormZ){
      currentLeftNormZ = baseLeftNormZ;
    }
    else{
      currentLeftNormZ = baseLeftNormZ + ((peakNormZ - baseLeftNormZ)/(peakY - baseLeftY))*((double) i - baseRightY);
    }
    ...
    if(peakNormX == baseRightNormX){
      currentRightNormX = baseRightNormX;
    }
    else{
      currentRightNormX = baseRightNormX + ((peakNormX - baseRightNormX)/(peakY - baseLeftY))*((double) i - baseRightY);
    }
    if(peakNormY == baseRightNormY){
      currentRightNormY = baseRightNormY;
    }
    else{
      currentRightNormY = baseRightNormY + ((peakNormY - baseRightNormY)/(peakY - baseLeftY))*((double) i - baseRightY);
    }
    if(peakNormZ == baseRightNormZ){
      currentRightNormZ = baseRightNormZ;
    }
    else{
      currentRightNormZ = baseRightNormZ + ((peakNormZ - baseRightNormZ)/(peakY - baseLeftY))*((double) i - baseRightY);
    }
    ...
    for(int j = leftBound; j <= rightBound; j++){
      ...
      currentNormX = ((currentRightNormX - currentLeftNormX)/(currentRightX - currentLeftX))*((double) j - currentLeftX) + currentLeftNormX;
      currentNormY = ((currentRightNormY - currentLeftNormY)/(currentRightX - currentLeftX))*((double) j - currentLeftX) + currentLeftNormY;
      currentNormZ = ((currentRightNormZ - currentLeftNormZ)/(currentRightX - currentLeftX))*((double) j - currentLeftX) + currentLeftNormZ;

      double normal[3];
      normal[0] = currentNormX;
      normal[1] = currentNormY;
      normal[2] = currentNormZ;

      double shading = calculatePhongShading(lp, viewDirection, normal);
      if(shading == 0.0){
        writeFlag |= true;
      }
      ...
      //more verbose debugging informationshading:
      if(verboseDebug){
      ...
      if((currentZ >= zVals[currentPixel]) && (currentZ <= 0.0) && (currentZ >= -1.0)){
        int red, green, blue;
        red = ceil441(255*currentRed*shading);
        green = ceil441(255*currentGreen*shading);
        blue = ceil441(255*currentBlue*shading);
...
int main()
{
  //width and height of the image
   ...
   double maxNormX, maxNormY, maxNormZ, midNormX, midNormY, midNormZ, minNormX, minNormY, minNormZ;
  ...
  //for(int j = 2; j<=2; j++){
    ...
    //otherwise split the triangle at the mid-Y value into a flat-top and flat-bottom triangle
      ...
      maxNormX = currentTriangle.normals[0][0];
      maxNormY = currentTriangle.normals[0][1];
      maxNormZ = currentTriangle.normals[0][2];
      midNormX = currentTriangle.normals[0][0];
      midNormY = currentTriangle.normals[0][1];
      midNormZ = currentTriangle.normals[0][2];
      minNormX = currentTriangle.normals[0][0];
      minNormY = currentTriangle.normals[0][1];
      minNormZ = currentTriangle.normals[0][2];
      ...
      for(int i = 1; i<3; i++){
        if(currentTriangle.Y[i] > maxY){
          ...
          maxNormX = currentTriangle.normals[i][0];
          maxNormY = currentTriangle.normals[i][1];
          maxNormZ = currentTriangle.normals[i][2];
        }
        if(currentTriangle.Y[i] < minY){
          ...
          minNormX = currentTriangle.normals[i][0];
          minNormY = currentTriangle.normals[i][1];
          minNormZ = currentTriangle.normals[i][2];
        }
      }
      ...
      for(int i = 0; i<3; i++){
        if((currentTriangle.Y[i] < maxY) && (currentTriangle.Y[i] > minY)){
          ...
          midNormX = currentTriangle.normals[i][0];
          midNormY = currentTriangle.normals[i][1];
          midNormZ = currentTriangle.normals[i][2];
        }
      }
      ...
      newNormX = minNormX + ((maxNormX - minNormX)/(maxY - minY))*(midY - minY);
      newNormY = minNormY + ((maxNormY - minNormY)/(maxY - minY))*(midY - minY);
      newNormZ = minNormZ + ((maxNormZ - minNormZ)/(maxY - minY))*(midY - minY);
      ...
      triangle1.normals[0][0] = maxNormX;
      triangle1.normals[0][1] = maxNormY;
      triangle1.normals[0][2] = maxNormZ;
      triangle1.normals[1][0] = midNormX;
      triangle1.normals[1][1] = midNormY;
      triangle1.normals[1][2] = midNormZ;
      triangle1.normals[2][0] = newNormX;
      triangle1.normals[2][1] = newNormY;
      triangle1.normals[2][2] = newNormZ;
      ...
      triangle2.normals[0][0] = minNormX;
      triangle2.normals[0][1] = minNormY;
      triangle2.normals[0][2] = minNormZ;
      triangle2.normals[1][0] = midNormX;
      triangle2.normals[1][1] = midNormY;
      triangle2.normals[1][2] = midNormZ;
      triangle2.normals[2][0] = newNormX;
      triangle2.normals[2][1] = newNormY;
      triangle2.normals[2][2] = newNormZ;
   ...
   WriteImage(image, "project1Efinished");
}
